...
- [ ] **Self-declaration as a copyright basis:** the product currently treats a `self_owned_declaration` (a bare attestation of ownership) as proof of ownership for `copyright`. Given the matrix's own warning against "a face in a photo the subject didn't take or own", what corroboration (originals with metadata, registration) should be required before a copyright notice may be sent on a self-declaration alone?
- [ ] **Identity verification vs. "supported":** Slice 2 derives `likeness`/`ncii`/`impersonation` support from enforcement consent + authorization only; the matrix also requires identity verification (Phase 2 liveness/ID). Confirm that "supported" as an internal readiness signal, with no notice filed until identity is verified, is acceptable.
- [ ] **Whole-image embeddings & biometric law:** Slice 3 stores a whole-image OpenCLIP embedding per reference photo for copy-matching (no face detection, cropping, or face-specific embedding). Are whole-image CLIP embeddings of photos that happen to contain a face "biometric identifiers/information" under Illinois BIPA or Texas CUBI? If yes, we must gate embedding on biometric consent and wire asset-embedding deletion into the consent-revocation purge hook.
